...
jQuery( function( $ ) {
    ...
    var wc_add_to_cart = {
            ...
            if($remove_link.data('product_id') && $remove_link.data('cart_item_key')){
                ...
                $.ajax({
                    ...
                    success: function( response ) {
                        ...
                        wc_add_to_cart.update_cart_count(response.data.count, response.data.subtotal);
        ...
        update_cart_count(count, subtotal){
            $('.wrech-cart-count').text(count);
            $('.wrech-subtotal strong').html(subtotal);
        },
    ...
    var wrech_modal = {
        ...
        $subtotal: $('.wrech-subtotal'),
        $car_btn: $('.wrech-cart-btn'),
        $order_info_btn: $('.wrech-order-info-btn'),
        $order_placeorder_btn: $('.wrech-placeorder-btn'),
        ...
        init: function() {
            ...
            this.$mask_overlay.on( 'click', this.hide_modal );

            this.$mask_overlay.css('display','none');
            ...
            this.$order_placeorder_btn.on( 'click', this.place_order );

            //Hide place order button when PayPal is checked
            $(document).on("change", "form[name='checkout'] input[name='payment_method']", this.place_order_btn_behaivor);
            $('.wc_payment_methods input[name="payment_method"]').on("change", function(){
                console.log('Tete')
            });

            //this.check_info_fields();

            wrech_modal.initialStepsState();
            wrech_modal.show_cart();

        },
        place_order_btn_behaivor(){
            if($('#payment_method_ppcp-gateway').is(':checked')){
                wrech_modal.$order_placeorder_btn.css('display', 'none');
            }else{
                wrech_modal.$order_placeorder_btn.css('display', 'block');
            }
        },
        check_empty_cart(){
        ...
        show_cart(){
            ...
            wrech_modal.$order_info_btn.css('display', 'block').addClass('wrech-active');;
            wrech_modal.$subtotal.css('display', 'block');
        ...
        show_info(){
            if(!wrech_modal.check_empty_cart()){
                ...
                wrech_modal.$car_btn.css('display', 'block');
                wrech_modal.$payment_btn.css('display', 'block').addClass('wrech-active');;
            }
            else{
        ...
        show_payment(){
            ...
            if(!wrech_modal.check_info_fields()){
                toastr.error('There are some fields with errors' , '');
                return;
            }
            ...
            if(!wrech_modal.check_empty_cart()){
                ...
                wrech_modal.$order_info_btn.css('display', 'block');
                wrech_modal.$order_placeorder_btn.css('display', 'block').addClass('wrech-active');
            }
        },
        hide_modal: function() {
            wrech_modal.$checkout_modal.addClass('wrech-slide-out');
            $('.wrech-modal-mask').css('display','none');
            setTimeout(() => {
        ...
        show_modal(){
            ...
            $('.wrech-modal-mask').css('display','block');
            //wrech_modal.$checkout_modal.animate({width: 'toggle'}, {duration: 500})
        },
        initialStepsState(){
            $('.wrech-step').hide();
            wrech_modal.$payment_btn.css('display', 'none').removeClass('wrech-active');
            wrech_modal.$car_btn.css('display', 'none').removeClass('wrech-active');
            wrech_modal.$order_info_btn.css('display', 'none').removeClass('wrech-active');
            wrech_modal.$subtotal.css('display', 'none').removeClass('wrech-active');
            wrech_modal.$order_placeorder_btn.css('display', 'none').removeClass('wrech-active');
        },
        place_order(){
        ...
        validate_phone(phone){
            ...
            phone.closest('.form-row').addClass('woocommerce-invalid').addClass('woocommerce-invalid-phone');
            return false
        },
        check_info_fields(){
            var fields = wc_checkout_params.required_fields;
            var billing_fields = fields['billing'];
            var shipping_fields = fields['shipping'];

            //Checking billing fields
            for(var i = 0; i < billing_fields.length; i++){
                var $field = $('#' + billing_fields[i]['key']);
                if($field.val() == ''){
                    //wrech_modal.scrollTo(billing_fields[i]['key']);
                    return false;
                }

                if(billing_fields[i]['type'] == 'email' && !this.validate_email($field.val())){
                    //wrech_modal.scrollTo(billing_fields[i]['key']);
                    return false;
                }

                if(billing_fields[i]['type'] == 'tel' && !this.validate_phone($field)){
                    //wrech_modal.scrollTo(billing_fields[i]['key']);
                    return false;
                }
            }

            if($('.wrech-main #ship-to-different-address-checkbox').is(':checked')){
                console.log('yes')
                //Checking billing fields
                for(var i = 0; i < shipping_fields.length; i++){
                    var $field = $('#' + shipping_fields[i]['key']);
                    if($field.val() == ''){
                        //wrech_modal.scrollTo( shipping_fields[i]['key']);
                        return false;
                    }

                    if(shipping_fields[i]['type'] == 'email' && !this.validate_email($field.val())){
                        //wrech_modal.scrollTo(shipping_fields[i]['key']);
                        return false;
                    }

                    if(shipping_fields[i]['type'] == 'tel' && !this.validate_phone($field)){
                        //wrech_modal.scrollTo(shipping_fields[i]['key']);
                        return false;
                    }
                }
